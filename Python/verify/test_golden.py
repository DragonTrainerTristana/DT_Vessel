...
주의
  ...
  - 골든 파일 Python/verify/golden/*.json 은 git 추적. --regen 은 diff 로 드러난다.
"""
import argparse
import hashlib
import json
...
# verify/ 로 내려온 뒤에도 학습기(vessel_gym_train.py)는 core-train 그룹이라 Python/ 루트에 그대로 있음
# (PLAN.md 8단계 risk: "test_golden 이 verify/ 로 내려가면 HERE 가 학습기를 못 찾음" — PYROOT 로 분리해 해소).
PYROOT = os.path.dirname(HERE)
GOLDEN_DIR = os.path.join(HERE, 'golden')
TRAIN = os.path.join(PYROOT, 'vessel_gym_train.py')
STAMP = '2026-09-10'
...
BATCH_ENV = {
    ...
    'VESSEL_DYN_PROFILE': 'agile', 'VESSEL_OBSTACLES': 'grid3x3',   # ★2026-09-21 프로필 핀(골든 생성 당시 값)
}
...
def run_case(name, spec):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        r = subprocess.run(cmd, env=env, cwd=PYROOT, capture_output=True, text=True, encoding='utf-8', errors='replace')
        if r.returncode != 0:
...
_YUGIOH_CONSTS = ['USE_ATTENTION', 'CENTRAL_CRITIC', 'STATE_RECON_COEF', 'USE_MOE', 'MOE_SHARED', 'MOE_WIDTH',
                  ...
                  'TIMEOUT_BOOTSTRAP', 'MSG_GATE_APPLY', 'DYN_PROFILE', 'OBSTACLES_MODE']
...
def _config_dump(extra_env):
    ...
    r = subprocess.run([sys.executable, '-c', code], env=env, cwd=PYROOT, capture_output=True, text=True, encoding='utf-8', errors='replace')

...
def load(prefix, frac=0.25):
    rows=[]
    for s in (42,43,44):
        d=glob.glob(os.path.join(RES, f'*_mt10_{prefix}_s{s}'))   # ★mt10 명시
        if not d: continue
...
for name,good in [('headingTravel','↓'),('fuelTurn','↓'),('fuelThrust','≈'),('commandVar','↓')]:
    signs=[]
    for s in (42,43,44):
        do=glob.glob(os.path.join(RES,f'*_mt10_commON_s{s}')); df=glob.glob(os.path.join(RES,f'*_mt10_commOFF_s{s}'))

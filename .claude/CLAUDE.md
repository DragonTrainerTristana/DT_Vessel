...
- `common_env()` 정본 = **`Python/common_env.sh` 한 파일**(2026-09-24 단일화 — `run_repro.sh`·`smoke_mac.sh` 가 `source` 함. 예전엔 사본 두 벌이라 export 하나를 두 번 고쳐야 했음) = **YUGIOH 를 명시 export**(config 기본값과 동일). `preflight` 가 export 값과 config 기본값을 대조해 드리프트면 중단. `${VAR:-기본}` 3개(`VESSEL_DYN_PROFILE`·`VESSEL_OBSTACLES`·`VESSEL_RADAR_RANGE`)는 바깥 override 를 보존하는 의도된 실험 축이라 그 `names`(= 기본값과 같은가) 목록에서는 빠져 있고, 대신 preflight 의 **override 축 검사**가 common_env 가 export 한 값을 config 가 실제로 읽었는지 대조하고(아니면 FAIL) 기본값과 다르면 ★ 로 찍는다(실행 헤더에도 `radar=` 로 표시). `VESSEL_RADAR_RANGE` 축(용량반응 스윕, 설계 스펙 §8-5)은 **저자 확인 대기** — 되돌리려면 `common_env.sh` 에서 56 으로 고정하고 `names` 에 `RADAR_RANGE` 를 되돌린다. 학습 인자 `--envs 128 --vessels 16 --rollout 32 --ring 1.0 --crossing 2 --max_partners 4 --steps 16056320`(= `config.YUGIOH_ARGS`; rollout 32 는 12런 aux.csv 245행 = update 당 65,536 결정으로 확인). 2026-09-04 배치 설정은 `verify/test_golden.py BATCH_ENV`(legacy 핀 포함)에만 남아 있음.

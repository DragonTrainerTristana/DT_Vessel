...
- [x] **0. 드리프트 검사를 common_env 31개 전부로 확장** — `a2473dd`
...
- [x] **1. shared + core-train 동결** (`run_repro.sh` `config.py` `networks.py` `vessel_gym.py` `vessel_gym_train.py` `ckpt_io.py` — 이동 없음) — **이동 없음 = 전용 커밋 없음** (동결 사유는 PLAN.md `미해결 항목` 참조)
      완료 조건: smoke PASS + `YUGIOH 드리프트: PASS` + config.py 를 건드렸으면 `test_golden.py --check` 가 `ALL PASS`.

- [x] **2. unity-island → `_archive/unity_path_2026-09/`** (`main.py` `frame_stack.py` `memory.py` `functions.py` `obs_utils.py`) — `dcaef15`
      완료 조건: smoke PASS(코어 무영향) + 끊긴 엣지 0 + `_archive` 에 사유·복구법 README.

- [x] **3. deprecated → `_archive/deprecated_2026-09/`** (`_smoke_fullmoe.py` `inspect_channel_freeze.py`) — `feb6791`
      완료 조건: smoke PASS + preflight 출력이 2단계와 동일 + README 에 `_smoke_fullmoe` 상시 FAIL 사실 기재.

- [x] **4. analysis → `Python/analysis/`** (`metric_io.py` + analyze 4종 + `convergence_gate.py`) — `3a5a36e`
      완료 조건: smoke PASS(무변화) + 이동한 6개 **각각 직접 실행**해 `metric_io` import 와 RUN_DIR 이 이전과 같은 절대경로로 해석되는지 확인.

- [x] **5. fidelity → `Python/fidelity/`** (`radar_fidelity_compare.py` `unity_fidelity_compare.py`) — `d1ae256`
      완료 조건: smoke PASS + 두 파일이 계산하는 Build 경로 문자열이 이동 전과 같은 곳을 가리킴(Build 실물 없으면 문자열만).

- [x] **6. plotting 위치 확정 + astar_fig9 편입** (`eval_astar_global.py` → `astar_fig9/`, `write_fig_code.py:13` 수정) — `fe507f8`
      완료 조건: `plotting/regenerate_all.py` 1회 완주 + `make_fig9_from_eval.py` 가 shim 경유로 `paper_style` 을 올림 + `eval_astar_global.py` CLI 1회 실행.

- [x] **7. eval → `Python/eval/`** (`eval_ckpt.py` `eval_mixed.py` `diag_ckpt.py` `measure_regimes.py` `corridor_run.py` `compose_voyage.py` + `run_repro.sh:183,269` 동시 수정) — `b0edb13`
      완료 조건: smoke PASS + `bash run_repro.sh eval` 1회 + `VESSEL_DIAG_CKPTS=… bash run_repro.sh diag` 1회(게이트 3개 통과해 JSON 산출).

- [x] **8. verify → `Python/verify/`** (`_verify_ppo_mirror.py` `_verify_comm_mirror.py` `test_golden.py` `test_vessel_gym_fidelity.py` `test_ckpt_compat.py` `golden/` + `run_repro.sh:122,123,131,135` 동시 수정) — `3af38d5`
...
| 날짜 | 단계 | 결과 | 커밋 |
|---|---|---|---|
| 2026-09-15 | 0~8 전체 (Windows 최종 검증) | `run_repro.sh smoke` EXIT=0 / 골든 5케이스 비트동일 / PPO 미러 Windows 최초 통과 / eval·diag 경로 확인 완료 | `aab2781` |
